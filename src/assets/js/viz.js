...
const formatInpus = (source, toggleContext) => {
  ...
  const formatContext = (index, hasDouble) => {
    const formattedName = hasDouble
      ? source.dataSource[index]
      : source.dataSource[0];
    const formatContext = toggleContext.context;

    let formattedLabels = formatContext.keepLabels
      ? formatContext.labels[0]
      : formatContext.labels[index];
    let formattedUnits = formatContext.keepUnits
      ? formatContext.units[0]
      : formatContext.units[index];
  ...
  if (toggleContext === "initial") {
    ...
    [labels, units, chartName] =
      toggleContext.doubleToggle > -1
        ? formatContext(toggleContext.doubleToggle, true)
        : formatContext(toggleContext.chartNumber, false);
...
const formatLabels = (y, x, context, y2) => {
  if (!y2)
    context.units
      ? y.tickFormat(d3.format(axisFormats[context.units]))
      : y.tickFormat(d3.format(".3n"));
  else
    context.units
      ? (y.tickFormat(d3.format(axisFormats[context.units])),
        y2.tickFormat(d3.format(axisFormats[context.units])))
...
const standardMargin = isMobile
  ? { top: 40, right: 25, bottom: 50, left: 67 }
  : { top: 40, right: 55, bottom: 45, left: 85 };
const waterfallMargin = isMobile
  ? { top: 38, right: 20, bottom: 330, left: 58 }
  : { top: 40, right: 55, bottom: 275, left: 85 };
  ...
  d3.csv(
    ...
    (rows) => {
      ...
      source.data.forEach((series) => {
        ...
        const y =
          rows[series.columns[1]] === "NA" || isNaN(rows[series.columns[1]])
            ? null
            : +rows[series.columns[1]];
    ...
    () => {
      nv.addGraph(() => {
        ...
        const isPercent =
          context.units === "percent" || context.units === "percentC"
            ? true
            : false;
...
const createLinePlusBarChart = (source, toggleContext) => {
  ...
  Array.from(chart.getElementsByClassName("nv-barsWrap nvd3-svg")).map((elem) =>
  ...
  let barSource =
    barIndex === 0 ? source.data[0].columns : source.data[1].columns;
  let lineSource =
    lineIndex === 1 ? source.data[1].columns : source.data[0].columns;
  d3.csv(
    dataSource,
    (rows) => {
      source.data[barIndex].values.push([
        +rows[barSource[0]],
        rows[barSource[1]] === "NA" ? null : +rows[barSource[1]],
      ]);
      source.data[lineIndex].values.push([
        rows[lineSource[1]] === "" || isNaN(rows[lineSource[1]])
          ? null
          : +rows[lineSource[0]],
        rows[lineSource[1]] === "" ? null : +rows[lineSource[1]],
      ]);
    },
    (csvObj) => {
      nv.addGraph(() => {
        ...
        let barMax = 0,
          lineMax = 0;
        source.data[barIndex].values.forEach(
          (val) => (barMax = Math.max(barMax, val[1]))
        );
        source.data[lineIndex].values.forEach(
          (val) => (lineMax = Math.max(lineMax, val[1]))
        );

        source.data[lineIndex].hideYAxis &&
    ...
    (rows) => {
      source.data.forEach((series) => {
        series.values.push([
          ...
          rows[series.columns[1]] === "NA" ? -1 : +rows[series.columns[1]],
        ]); // set -1 to hide null values
      });
    },
    (csvObj) => {
      nv.addGraph(() => {
...
const createLineAndScatterChart = (source, toggleContext) => {
  ...
  let scatterSource =
    scatterIndex === 0 ? source.data[0].columns : source.data[1].columns;
  let lineSource =
    lineIndex === 1 ? source.data[1].columns : source.data[0].columns;
...
const createWaterfallChart = (source, toggleContext) => {
  ...
  var xAxis = () =>
    d3.svg.axis().scale(x).orient("bottom").ticks(20).tickSize(-height, 0, 0);
  var yAxis = () =>
    d3.svg.axis().scale(y).orient("left").ticks(8).tickSize(-width, 0, 0);
  ...
  d3.csv(dataSource, (data) => {
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      if (i % 3 === 0) {
        ...
        val > 0
          ...
          : (data[i].class = "waterfall-orange"); // stylize if positive or negative
      }
    ...
    chart
      ...
      .attr(
        "transform",
        "translate(" + -margin.left / 1.3 + "," + height / 2 + ")rotate(-90)"
      )
    ...
    chart
      ...
      .attr(
        "transform",
        "translate(" + width / 2 + "," + (height + xLabelMargin) + ")"
      )
